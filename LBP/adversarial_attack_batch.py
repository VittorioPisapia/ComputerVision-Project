...
with open('D:\CV\Models_and_scalers\lbp_svm_model.pkl', 'rb') as f:
    svm = pickle.load(f)

with open('D:\CV\Models_and_scalers\scaler_LBP.pkl', 'rb') as f:
    scaler = pickle.load(f)

classifier = SklearnClassifier(model=svm)

_, X_test, _, y_test = preprocess_data_with_lbp('D:/CV/dataset/0', 'D:/CV/dataset/1')
...
attack = FastGradientMethod(estimator=classifier, eps=0.03) #Default parameters: eps=0.03
#attack = ProjectedGradientDescent(estimator=classifier, eps=0.02, max_iter=10) #Default parameters: eps=0.02, max_iter=10
#attack = CarliniL2Method(classifier=classifier, confidence=0.1, max_iter=20) #Default parameters: confidence=0.1, max_iter=20
